...
def _get_arm_qpos_near_object(env, target_obj_name: str) -> np.ndarray:
    ...
    # Target position is the object position (0cm offset)
    # target_pos = obj_pos + np.array([0.0, 0.0, 0.08])
    offset = np.array([
...
def _get_moveable_object_poses_6d(env, movable_objects: list) -> np.ndarray:
    """Return 6D poses (x, y, z, axis-angle) for all moveable objects, shape (len(movable_objects), 6)."""
    model = env.sim.model._model
    data = env.sim.data._data
    poses = []
    for obj_name in movable_objects:
        body_name = env.env.objects_dict[obj_name].root_body
        body_id = model.body(body_name).id
        pos = data.xpos[body_id].copy()
        # MuJoCo xquat is (w, x, y, z); _quat2axisangle expects (x, y, z, w)
        quat_wxyz = data.xquat[body_id]
        quat_xyzw = np.array([quat_wxyz[1], quat_wxyz[2], quat_wxyz[3], quat_wxyz[0]])
        orientation_axis_angle = _quat2axisangle(quat_xyzw)
        poses.append(np.concatenate([pos, orientation_axis_angle]))
...
class Args:
    ...
    num_trials_per_task: int = 200  # Number of rollouts per task; there are 50 sampled init states
    data_collection_start_idx = 0
    #################################################################################################################
    # Utils
    #################################################################################################################
    video_out_path: str = "data/libero/videos/arm_near_object/task_1/test_100_josh"  # Path to save videos
    data_saving_path: str = 'examples/libero/dataset_task1_test_100_josh.hdf5'

    seed: int = 12  # Random Seed (for reproducibility) [seed 12 for train data, seed 10 for test data]
...
def collect_rollout(args: Args) -> None:
    ...
    with h5py.File(args.data_saving_path, 'w') as hdf5_file:
        ...
        for task_id in [1]:
            ...
            logging.info(f"Found movable objects: {movable_objects}")
            ...
            # for trial_id in range(args.num_trials_per_task):
                ...
                trajectory_states = []
                trajectory_actions = []
                trajectory_object_poses = []  # 6D poses (T, num_objects, 6) for moveable objects
                ...
                # Prepare for contact check
                bowl_name = "akita_black_bowl_1"
                sustained_contact = True
                
                while t < max_steps:
                    try:
                        # Check for sustained contact in the last 20 steps
                        if t > max_steps - 20 and sustained_contact:
                            if not check_gripper_bowl_contact(env, bowl_name):
                                sustained_contact = False

                        # # Get preprocessed image
                        # # IMPORTANT: rotate 180 degrees to match train preprocessing
                        # img = np.ascontiguousarray(obs["agentview_image"][::-1, ::-1])
                        # # Split task_description into multiple lines if too long
                        # max_chars = 40
                        # words = task_description.split()
                        # lines = []
                        # current_line = "step #{:d}:".format(t)
                        ...
                        # # Draw text on image with better readability
                        # y0, dy = 30, 30  # Increased spacing
                        # font_scale = 0.35  # Doubled from 0.3
                        # thickness = 1     # Increased from 1
                        # for i, line in enumerate(lines):
                        #     cv2.putText(img, line, (10, y0 + i * dy), 
                        #                 cv2.FONT_HERSHEY_SIMPLEX, font_scale, 
                        #                 (255, 0, 0), thickness, cv2.LINE_AA)
                        
                        # plt.imsave('examples/libero/figs/arm_group_{:d}_num_{:d}.png'.format(arm_group_idx, trial_id // num_arm_groups), img)
                        # trial_id += 1
                        # break
                        ...
                        trajectory_object_poses.append(_get_moveable_object_poses_6d(env, movable_objects))
                ...
                if not done and sustained_contact:
                    logging.warning(f"Episode failed but gripper maintained contact with bowl for last 20 steps (indicates it was carrying and would succeed). Resampling...")
                    continue
                ...
                if len(trajectory_images) > 10:  # Only save if we collected data
                    ...
                    # 6D poses (x,y,z, axis-angle) per moveable object, shape (timestep, num_objects, 6)
                    ep_grp.create_dataset(
                        "observations/object_poses_6d",
                        data=np.array(trajectory_object_poses),
                    )
                    ep_grp.attrs["movable_objects"] = np.array(movable_objects, dtype="S")  # order matches dim 1 of object_poses_6d
